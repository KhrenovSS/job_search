...
- **crawl** — одноразовый DateTrigger: при `kv.paused` — уведомление и сразу план следующего подхода; иначе
  ...
  (подход восстановлен после долгого простоя) — «⏭ Подход пропущен», план следующего. Перед запуском `next_crawl_at`
  очищается (индекс и дата окна остаются — рестарт посреди сбора не назначит это окно снова), после прогона — план следующего
  подхода и отчёт с его временем. Ручной `/crawl [N]` план и дедлайн не трогает, берёт весь остаток лимита, N ограничивает
  страницы. Ошибка браузера: ретраев нет — следующий подход через несколько часов, владельцу напоминание про `--marionette`.
