...
- **digest** — cron `DIGEST_TIME` (12:00): сначала `_evaluate_pending` (дооценить всё `prefiltered` и дописать письма —
  чтобы утренний сбор, который ещё догружает описания, успел попасть в дайджест частично), затем `send_digest()`, затем
  `plan_next_crawl()` — случайный момент **старта** в `CRAWL_WINDOW` (07:00–08:30), не раньше «сейчас + 20 мин»;
...
(проверено M вакансий)» (+ строка «Необработанных с прошлых дней: K (/inbox)», если K > 0) → на каждый лид карточка (`ranker.format_card`, клавиатура 👍/👎) + письмо (`format_letter`, `<pre>`),
...
- `/inbox` — открытые лиды (старые сначала, отложенные внизу); `/done <hh_id>` — как ✅; `/cleanup [дней=14]` —
  открытые старше N дней → `closed_stale` («⌛ Устарело»). Автоматического устаревания нет.
