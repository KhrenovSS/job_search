...
- Старт сервиса: `next_crawl_at` в будущем → зарегистрировать job на **то же** сохранённое время; в прошлом → запустить
  через 2–5 мин (проверки «уже был сегодня» нет — рестарт после пропущенного времени даёт сбор); ключа нет →
  `plan_on_start`: если сегодняшнее окно ещё впереди и в `runs` нет прогона за сегодня — назначить сбор на сегодня
  (первый старт после установки, исчерпанные повторы); иначе до ближайшего дайджеста сборов не будет (ручной `/crawl`).
