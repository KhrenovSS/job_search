...
Синхронизация читает `NEGOTIATIONS_PAGES` страниц списка (по 20 записей) — по одной за вызов `step()`, с выходом
при пустой странице, при повторе id и по `paging`; каждая смена состояния пишется в `negotiation_events`.
`/stats` считает только письма старше `OUTCOME_MATURE_DAYS` и печатает разрезы по признакам вакансии и письма,
пряча долю в ячейках меньше `repo.MIN_CELL` (решение №48).
...
письмо уже написано (`run.py` шаг 6), уходит владельцу **сразу после подхода**, а не ждёт полудня. Берётся верх
`repo.lead_queue` в пределах остатка суточной нормы (`DIGEST_MAX_ITEMS` − `repo.leads_sent_today`). Лид без письма
(мост подвёл) остаётся в очереди — минуты обращений к мосту не держат чат; так же придерживается лид, чьё письмо
написано по прежним правилам (`usable_letter`): его перепишет ближайший подход или дайджест 12:00. Запись — `digests` с `note='instant:<site>'`
