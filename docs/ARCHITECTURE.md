...
- `plant` — **эксплуатанты автоматики** (v9.15, решение №55, `pipeline/plant.py`): триаж карточек вакансий
  ...
  из пула `PLANT_LEADS_PER_DAY` (40) строк в день → `to_fetch` с приоритетом 3 (после вакансий программистов; страховка
  `low_priority_expired`). Дальше — путь v9.13: страница вакансии как описание предприятия, `company_evaluation.md`
...
всю очередь по приоритету (с v9.14 нормы нет; `DIGEST_MAX_ITEMS` > 0 возвращает потолок). Лид без готового письма не уходит и остаётся `evaluated`
