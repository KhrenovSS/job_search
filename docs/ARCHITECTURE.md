...
| 2b | Одна компания — один лид | `pipeline/dedup.py` | Сначала `revive_orphans`: дубли, чей «победитель» ушёл в `rejected`/`evaluation_failed`/`skipped` и у компании лида не осталось, возвращаются в очередь (`to_fetch`, если триаж ИИ уже пройден, иначе `triage`). Затем `skip_covered`: `triage`, чей работодатель уже имеет лид (`sent` за `EMPLOYER_REPEAT_DAYS`, `prefiltered`, `evaluated ≥ порог`) → `skipped/duplicate_employer:<hh_id>` — без триажа ИИ | — |
