...
со своим соединением SQLite (WAL, busy_timeout 30 с). Внутри процесса сборы сериализует `asyncio.Lock`; между процессами —
...
| 5b | Досье на компанию | `llm/company_research.py` | Перед письмом, для лидов hh и каталога ОВЕН (`row_site in (hh, owen)`): CLI с веб-инструментами читает `hh.ru/employer/<id>` и сайт компании → `employers` (1 вызов на **компанию**, кэш `COMPANY_RESEARCH_TTL_DAYS`). Мимо браузера — дневной лимит загрузок не тратится. Сбой или `found: false` не мешает письму | мост (веб) |
...
6 % карточек за четверть лимита страниц, v9.15), в случайном порядке; пагинация до `max_pages_per_query` (6) с ранней
остановкой, когда страница не даёт новых — с v9.19 и первая: выдача по времени публикации, и если 50 самых свежих
карточек уже известны, остальные тоже (ночью это экономит ~10 страниц за подход в пользу страниц вакансий). Браузерные шаги идут **сериями по времени** (`browser/bursts.py`,
...
- `plant` — **эксплуатанты автоматики** (v9.15, решение №55, `pipeline/plant.py`): триаж карточек вакансий
  ...
  не умирает: `plant.pool` переводит её в пул `skipped/plant_pool` с `lead_kind='company'`, `search_pass='plant'` (одна
  строка на работодателя; компания с лидом или с ответом владельца не берётся). С v9.19 в пул кладёт и оценщик
  (`VacancyEvaluation.plant`, `plant.pool_evaluated`): открытая триажем вакансия, которая оказалась эксплуатацией на
  заводе с автоматикой и набрала меньше порога, — 53 из 73 открытых вакансий 24.09 пропадали как `rejected`; такая
  строка приносит в пул уже прочитанную страницу. Шаг 4c прогона (`plant.admit`) выпускает
...
- Контакты компании (v9.15): разведка (`company_research.md`) записывает в досье `website`, `contact_email`, `contact_phone` —
  общие адреса с сайта или страницы hh, без личных; карточка компании берёт их из досье, если в `raw_json` контактов нет
  (`ranker._dossier_contacts`).
Дальше — общая очередь, мгновенная отправка и дайджест. Письмо — `prompts/company_offer.md`
...
- **Подходы** (`CRAWL_WINDOWS`, по умолчанию 07:00–10:00, 12:00–15:00, 18:00–22:00; у владельца с v9.15 — шесть окон
  круглые сутки 00–03/04–07/08–11/12–15/16–19/20–23, решение №55; резервная копия в 03:40 — в зазоре между первым
  и вторым окном): один плановый сбор на окно.
...
- Старт сервиса (`_restore_crawl`): `next_crawl_at` в будущем → тот же job, **но** план на другой день при свободном
  окне сегодня (нет `sitting_done` за это окно, до конца окна хватает на `MIN_LEAD_MINUTES` 20 + `MIN_SITTING_MINUTES` 60) заменяется подходом сегодня; в прошлом →
...
`Scheduler.backup_job` в 03:40 зовёт `db.backup` (штатный SQLite backup API — безопасно на ходу), кладёт
...
`repo.lead_queue` берутся все лиды с годным письмом (с v9.14 нормы нет; при `DIGEST_MAX_ITEMS` > 0 — её остаток минус
`repo.leads_sent_today`). Лид без письма (мост подвёл) остаётся в очереди — минуты обращений к мосту не держат чат;
так же придерживается лид, чьё письмо написано по прежним правилам (`usable_letter`, отпечаток — по `rows.letter_key` строки: hh / profi / company):
...
пауза `TELEGRAM_PAUSE_S` (1.2 с); в `QUIET_HOURS` (23–07) — без звука (`disable_notification`; тревоги через `Notifier` звучат всегда). Затем `sent` + `digests/digest_items`; всё `evaluated` ниже порога → `rejected` (кроме строк с
...
получает строку «📉 Ниже порога, взят по дневному минимуму», шапка — «Добрано по дневному минимуму (ниже порога): N».
Пусто → «Сегодня лидов не нашлось.
Проверено M новых вакансий.» M = число оценок с прошлого полуденного дайджеста (`daily_only=True`, мгновенные отправки не в счёт). При первом старте сервиса уже показанные превью
...
- Callback-протокол: `fb:<id>:up|down`, `fbr:<id>:<reason|skip>`, `act:<id>:responded|later`, `noop`. Подписи кнопок
  ...
  ✅ отвечает «Лид уже закрыт» (клавиатура остаётся), причина 👎 замораживает клавиатуру (`done_kb`) с ответом «Учтено», ⏸ открытость лида не проверяет. Уведомления: «✅ Свернул N лид(ов)…», «⚠️ Автозакрытие лидов не выполнено».
...
`/next` — время следующего сбора · `/pause` `/resume` · `/skipped [N≤50]` (по умолчанию 15) — последние отсеянные · `/stats [дней]` (30, ≤365) · `/letter <hh_id> [пожелание]` —
