...
| 4c | Эксплуатанты | `pipeline/plant.py` | `plant.admit`: `PLANT_LEADS_PER_DAY` (40) строк из пула `skipped/plant_pool` → `to_fetch` приоритета 3 (страницы — в следующем подходе, по доле канала) | — |
...
- `plant` — **эксплуатанты автоматики** (v9.15, решение №55, `pipeline/plant.py`): триаж карточек вакансий
  ...
  строка на работодателя; компания с лидом или с ответом владельца не берётся). Шаг 4c прогона (`plant.admit`) выпускает
  ...
  любыми; `dedup.same_company` не склеивает вакансию и компанию одного работодателя.
...
`plan_digest` в `DIGEST_TIME` (12:00) — после `expire_queue` (`QUEUE_TTL_DAYS`, только в полдень) и `skip_unreachable`
(компания без e-mail → `skipped/no_email`, решение №56; то же перед мгновенной отправкой) — берёт всю очередь по приоритету (с v9.14 нормы нет; `DIGEST_MAX_ITEMS` > 0 возвращает потолок). Лид без готового письма не уходит и остаётся `evaluated`
