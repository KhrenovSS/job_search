...
`BRIDGE_WEB_TIMEOUT` (420 с против 150 с обычного), клиент ждёт `COMPANY_RESEARCH_TIMEOUT_S` (450 с) и **без
ретраев**: мост, только что не уложившийся в тяжёлую страницу, не уложится и со второй попытки. Объём работы
ограничен с двух сторон — промпт разрешает 3 обращения к сети, `COMPANY_RESEARCH_MAX_PER_RUN` (5) не даёт
одному прогону писем уйти в чтение веба надолго; недоисследованные компании достаются следующему прогону.
...
`BRIDGE_PORT=8766` (только для unit-файла), `BRIDGE_WEB_TIMEOUT=420` (таймаут веб-запросов разведки),
...
разведка по компании: 1 на **новую** компанию, sonnet, до 4 ходов и 3 обращений к сети (~$0.05–0.09, замер на ФомЛайн: 5 ходов,
300 с, $0.088 — после чего потолок ходов снижен до 4, а промпт ограничен тремя обращениями к сети).
Типичный день: 3–5 триаж + 5–12 оценка + 5–20 писем + 5–15 разведок ≈ $2–3.5 по счётчику моста (подписка).
