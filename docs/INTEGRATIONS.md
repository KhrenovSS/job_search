...
- На каждый прогон: `geckodriver --connect-existing --marionette-port 2828 --port <свободный>` (дочерний процесс),
...
- Человекоподобие: пауза 4–12 с после каждой загрузки (каждая ~8-я — 20–40 с), прокрутка на случайную глубину,
  случайный порядок запросов и проходов, ≤ 80 загрузок **в день** суммарно, один сбор в сутки в случайное время,
  серии 3–7 страниц с паузами 10–40 мин.
- Ошибки: Marionette не отвечает / geckodriver нет → `BrowserUnavailable` → уведомление владельцу, повтор позже.
