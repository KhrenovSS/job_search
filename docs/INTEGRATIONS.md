...
писем складывает оба клиента. Объём работы ограничен с двух сторон — промпт разрешает 4 обращения к сети
(`COMPANY_RESEARCH_MAX_TURNS`=5), `COMPANY_RESEARCH_MAX_PER_RUN` (5) не даёт одному прогону писем уйти
в чтение веба надолго; недоисследованные компании достаются следующему прогону.
...
разведка по компании: 1 на **новую** компанию, opus, до 5 ходов и 4 обращений к сети (~$0.2–0.3 и 1–3 мин;
на sonnet было ~$0.05–0.09 и 20–45 с — разница сознательно оплачена качеством письма, v9.2).
Типичный день: 3–5 триаж + 5–12 оценка + до 5 писем + 5 разведок ≈ $3–5 по счётчику моста (подписка).
