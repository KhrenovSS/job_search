...
`LOW_PRIORITY_TTL_DAYS` (3) списываются (`skipped/low_priority_expired`). Ручной `/crawl` и CLI берут весь остаток дня
(`--budget N` — задать явно). Ритм: `BURST_MINUTES` (7–13) листаем / `GAP_MINUTES` (4–9) тишина; 120 загрузок ≈ 3 часа
