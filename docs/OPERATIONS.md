...
| `hh_scout.llm.evaluator [--limit N] [--preview] [--send] [--tail] [--requeue-rejected [--min-total N]] [--requeue-id HH_ID …]` | оценка: `prefiltered → evaluated`; `--send` шлёт превью в Telegram **напрямую, без кнопок и без записи в digests** — статусы не меняет (лиды остаются `evaluated` и попадут в следующий дайджест). `--requeue-rejected` (v8.7) возвращает на переоценку отклонённые с баллом ≥ `--min-total` (45), `--requeue-id` (v8.8, можно повторять) — названные вакансии в любом статусе: описания уже в `raw_json`, поэтому нужен только мост — браузер и лимит загрузок не трогаются. Ручная операция после правки промпта оценки; `--requeue-id` удобен, чтобы проверить правку на известном примере | — | да |
...
остатка, вечер — всё). Внутри прогона пул делится: доля `DETAILS_BUDGET_SHARE` (0.4) **резервируется под страницы вакансий** —
...
`LOW_PRIORITY_TTL_DAYS` (3) списываются (`skipped/low_priority_expired`). **Одна компания — один лид** (v8): вакансии работодателя,
