...
`LOW_PRIORITY_TTL_DAYS` (3) списываются (`skipped/low_priority_expired`). **Очередь лидов** (v9.1): оценённые выше порога складываются в очередь, дайджест берёт сверху `DIGEST_MAX_ITEMS` (5) по приоритету «балл + сутки ожидания, не больше `QUEUE_WAIT_BONUS_MAX`=7»; остальные ждут и видны в хвосте дайджеста (`DIGEST_TAIL_ITEMS`=10, письмо по `/letter <id>`), а через `QUEUE_TTL_DAYS`=30 выбывают (`rejected/queue_expired`). Письма пишутся только для суточной нормы — не на каждый прогон, а на день. **Одна компания — один лид** (v8): вакансии работодателя,
