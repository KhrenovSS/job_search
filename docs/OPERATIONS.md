...
| «database is locked» в стадии прогона (`описания: database is locked`) | другой процесс держал запись дольше `busy_timeout` — чаще всего массовая правка БД (CLI `--backfill`, `--requeue`, ручной SQL) во время идущего подхода (21.09, прогон #43) | не делать массовых записей при `runs.status='running'` — подходы теперь круглые сутки, проверять `runs` перед любым CLI с записью; `busy_timeout` поднят до 30 с, разовый сбой стадии переживается сам: строки `to_fetch` остаются на следующий подход |
