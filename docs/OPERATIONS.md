...
| `hh_scout.llm.evaluator [--limit N] [--preview] [--send] [--tail] [--requeue-rejected [--min-total N]]` | оценка: `prefiltered → evaluated`; `--send` шлёт превью в Telegram **напрямую, без кнопок и без записи в digests** — статусы не меняет (лиды остаются `evaluated` и попадут в следующий дайджест). `--requeue-rejected` (v8.7) возвращает на переоценку отклонённые с баллом ≥ `--min-total` (45): описания уже в `raw_json`, поэтому нужен только мост — браузер и лимит загрузок не трогаются. Ручная операция после правки промпта оценки | — | да |
