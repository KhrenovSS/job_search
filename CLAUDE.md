...
письмо ценно тем, что ушло раньше других. Суточная норма — **20 лидов и писем** (`DIGEST_MAX_ITEMS`, общая на
мгновенные отправки и дайджест; не предложить — гарантированный отказ, а окупаемость полос считает `/stats`).
...
и панель под их шкафы, типовые программы, ПНР и SCADA, подбор ПЛК; интеграторам — субподряд). Каналы — `COMPANY_CHANNELS`,
каталог входит по `COMPANY_LEADS_PER_DAY` в день, всё остальное — общая очередь и норма.
Отбор — **сквозная очередь**: оценённые выше порога не сгорают за сутки, а ждут своей очереди по приоритету
«балл + сутки ожидания» (`repo.lead_queue`); при норме 20 и нынешнем потоке 8–12 лидов в сутки очередь почти всегда
пуста — она осталась предохранителем на случай наплыва. Не попавшие в норму видны в хвосте дайджеста, письмо к ним —
...
⏰ **Отложенное решение владельца: дневной лимит страниц** (`DAILY_PAGE_LOADS` 150–200 выбирается полностью,
больше вакансий — только подняв его). Вернуться не раньше **2026-09-25**, критерии и запросы — `docs/ROADMAP.md`,
дело **0-cap**. Сам лимит не менять: это решение о риске, его принимает владелец.
