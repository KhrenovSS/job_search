...
Присылает в Telegram лиды **сразу после каждого подхода** (утром, днём и вечером), не дожидаясь полудня: готовое
...
go_hh.sh (быстрый разбор суток: сводка, прогоны и отправки за 24 ч, лиды по каналам, «ушло без письма», очередь и
         пул эксплуатантов, стадии из журнала; только чтение, границы суток — местные, а не UTC)
...
отдельной колонкой «без канала измерения». Порог **50** (решение №52, 20.09: владелец — «лучше откликнуться
...
Финал работает на отложенный контакт: вторая фраза называет конкретный повод вернуться к подряду позже, подпись — четыре строки (имя, «работаю по договору (ИП)», телефон, e-mail; решение №58: заказчику часто проще
...
электромобилей (пилот, как текущая работа); здания — вентиляция и диспетчеризация; фарма;
нефтегаз — без предложения проектирования): «вы проектируете насосные — ПО для КНС и насосных писал на ОВЕН/Schneider
