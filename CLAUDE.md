...
работы, и письмо предлагает сделать программную часть удалённо, а на пуск приехать. **Единственная обязательная чужая
среда — не лид** (решение №61, 25.09): «только Siemens / TIA Portal», «только Allen-Bradley», Omron, Mitsubishi, B&R —
компании нужен специалист по ней; флаг `foreign_platform_only` оценщика → `skipped/foreign_platform_only` независимо
от баллов. Перечень платформ или Siemens рядом с ОВЕН/CODESYS — по-прежнему лид.
...
энергетика триаж закрыл (`plant: true` в вердикте) или оценщик оценил ниже порога как эксплуатацию (`plant: true`
в оценке, v9.19 — страница уже прочитана и второй раз не грузится) — компания уходит в пул (`skipped/plant_pool`, `pipeline/plant.py`),
